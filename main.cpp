...
IMAGE	imgZmDead[20];
IMAGE	imgZmEat[21];
IMAGE	imgZmStand[11]; // 僵尸站立动画帧
IMAGE	imgSunShineBall[29];	//阳光序列帧总数

int curX, curY;	//当前选中植物在移动中的坐标
int curZhiWu;	//当前选中的植物	0-没有选中，1-选中第一种植物依次类推
...
struct sunShineBall {	//阳光球结构体
	...
	vector2 p1, p2, p3, p4;  // 贝塞尔曲线 p1起点， p2，p3控制点， p4终点
...
//声明场景巡场实现
void viewScence() {
	int xMin = WIN_WIDTH - imgBg.getwidth(); // 窗口宽度-图片宽度 900 - 1400=-500
...
void createSunShine() {
	...
	if (count >= fre) {	//限制阳光生成的速度
		...
		balls[i].t = 0;
		balls[i].p1 = vector2(260 - 112 + rand() % (900 - 320 + 112), 60);  // 随机落点
		balls[i].p4 = vector2(balls[i].p1.x, 200 + (rand() % 4) * 90);      //随机停止位置
		int off = 4;  //每次移动多少像素 
	...
	for (int i = 0; i < 3; i++) {
		for(int j = 0; j < 9; j++) {
			if (map[i][j].type == XIANG_RI_KUI + 1) {
				map[i][j].timer++;
				if (map[i][j].timer > 200) // 生产时间到了
				{
					map[i][j].timer = 0;

					int k;
					for (k = 0; k < ballMax && balls[k].used; k++);// 寻找可用的阳光球资源
					if (k >= ballMax)return;

					balls[k].used = true;

					balls[k].p1 = vector2(map[i][j].x, map[i][j].y);	//设置贝塞尔曲线的参数
					int w = (50 + rand() % 51) * (rand() % 2 ? 1 : -1); // 或许落在向日葵左边也许是右边【50~100】距离
					balls[k].p4 = vector2(map[i][j].x + w, map[i][j].y + imgZhiWu[XIANG_RI_KUI][0]->getheight()
						- imgSunShineBall->getheight()); // 设置阳光落点坐标
	...
	for (int i = 0; i < ballMax; i++) {
		if (balls[i].used) {
			...
			if (balls[i].status == SUNSHINE_DOWN) {
				...
				sun->t += sun->speed;
				sun->pCur = sun->p1 + sun->t * (sun->p4 - sun->p1); // 直线降落过程 p1 + t*(p4-p1)匀速运动;
				if (sun->t >= 1) {
				...
				struct sunShineBall* sun = &balls[i]; // 仅仅是方便后续程序操作定义的一个指针
				sun->t += sun->speed;
	...
	for (int i = 0; i < ballMax; i++) {
		if (balls[i].used) {	//阳光球被使用了才进行操作
			...
			if (msg->x > x && msg->x<x + w &&		//只有当光标在阳光范围内才进行操作
				...
				balls[i].p4 = vector2(262, 0); //如上设置的终点， 直线运动只需要起点终点两个坐标
				balls[i].t = 0;
				float distance = dis(balls[i].p1 - balls[i].p4); // p1 - p4点，获得P1指向P4的向量，再取模从而获得距离（像素长度）
				float off = 20; // 每次移动的像素
				balls[i].speed = 1.0 / (distance / off);
				break; // 一次搜集一个阳光
			}
		}
	...
	for (int i = 0; i < 3; i++) {	//3和9固定值，扩展性差，还是建议宏定义
		for (int j = 0; j < 9; j++) {
			if (lines[i] && map[i][j].type == 1) {	//有豌豆且僵尸走到打击范围
				map[i][j].shootTimer++;
				if (map[i][j].shootTimer > 20) {// 控制植物攻击间隔
...
void checkZm2ZhiWu() {
	for (int i = 0; i < zmMax; i++) {
		...
		int row = zms[i].row;
		for (int k = 0; k < 9; k++) {
			// 检查当前地图位置是否有植物  k也就是植物的位置
			if (map[row][k].type == 0)continue;

			int zhiWuX = 256 - 112 + k * 81;	//定义僵尸开吃范围
			int x1 = zhiWuX + 10;
			int x2 = zhiWuX + 60;
			int x3 = zms[i].x + 80; // 僵尸图片左侧有空白，此处作用是消去空白部分
			...
			if (x3 > x1 && x3 < x2) {
				...
				else {	//第一次被抓住  第一口吃植物了  僵尸开吃-配置参数
					map[row][k].catched = true;
					map[row][k].deadTimer = 0;
					zms[i].eating = true;
					zms[i].speed = 0; // 原地不动吃植物
					zms[i].frameIndex = 0;  // 设置僵尸eating帧序号
				}
			}
		}
	}
